...
def train_xgb (train: pd.DataFrame, test: pd.DataFrame):
    ...
    prediction = model.predict(matchup.iloc[0][train_stat_cols])[0]
    prob = model.predict_proba(matchup.iloc[0][train_stat_cols])
    platt_prob = platt_calibrated_model.predict_proba(matchup.iloc[0][train_stat_cols])
    isotonic_prob = isotonic_calibrated_model.predict_proba(matchup.iloc[0][train_stat_cols])
    fighter_red = matchup.iloc[0][train_stat_cols]['fighter_red'].iloc[0]
    fighter_blue = matchup.iloc[0][train_stat_cols]['fighter_blue'].iloc[0]

    if prediction == 1:
        answer = fighter_red
    else:
        answer = fighter_blue
    print (f"The model predicted: {answer} to win with probabilities {prob} (Uncalibrated), {platt_prob} (Platt), {isotonic_prob} (Isotonic Regression)")
