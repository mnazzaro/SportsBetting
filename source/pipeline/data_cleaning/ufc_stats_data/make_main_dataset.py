...
def _make_fighter_tott_df (fighter_cumulative_df: pd.DataFrame, fighters_df: pd.DataFrame) -> pd.DataFrame:
    def _update_features (group):
        try:
            url =  group['url'].iloc[0]
            wins = fighters_df[fighters_df['url'] == url]['wins'].iloc[0]
            losses = fighters_df[fighters_df['url'] == url]['losses'].iloc[0]
            w_l = []
            l_l = []
            for i in group['outcome'].tolist()[::-1]:
                if i == 'W':
                    wins -= 1
                    w_l.append(wins)
                    l_l.append(losses)
            ...
            try:
                dob = datetime.strptime(fighters_df[fighters_df['url'] == url]['dob'].iloc[0], '%b %d, %Y')
                group['age'] = group['date'].map(lambda x: (x - dob).days)
            except:
                group['age'] = group['date'].map(lambda x: np.nan)
            return group
        except Exception as e:
            print (f'No data for {group["fighter"].iloc[0]} with {e}')
            return None
        
    updated_record_df: pd.DataFrame = fighter_cumulative_df.groupby('url').apply(_update_features).reset_index(drop=True)
    updated_record_df['wl_percentage'] = updated_record_df['wins'] / updated_record_df['losses']
    return updated_record_df
...
def make_main_dataset (fighters_df: pd.DataFrame, fight_stats_df: pd.DataFrame, 
    ...
    moving_tott_df = _make_fighter_tott_df(fighter_cumulative_df, fighters_df)
    ...
    def _get_cum_fighter_stats (fighter: str, url: str, date: datetime, suffix: str) -> pd.DataFrame:
        ...
        tott = fighters_df[fighters_df['url'] == url][['weight', 'height', 'reach', 'stance_open_stance', \
                                                               'stance_orthodox', 'stance_sideways', 'stance_southpaw', \
                                                                'stance_switch']].squeeze()
        base_stats = fighter_cumulative_df[(fighter_cumulative_df['url'] == url) & (fighter_cumulative_df['date'] < date)][stat_columns]
        moving_tott = moving_tott_df[(moving_tott_df['url'] == url) & (moving_tott_df['date'] == date)][['age', 'wins', 'losses', 'wl_percentage']].squeeze()
        ...
        non_quant_stats = fighter_cumulative_df[(fighter_cumulative_df['url'] == url) & (fighter_cumulative_df['date'] == date)][non_quant_stat_columns].squeeze()
        days_temp = fight_stats_df[(fight_stats_df['url'] == url) & (fight_stats_df['date'] == date)]
